...
def _log(message: str, level: Union[info, error, warning] = info) -> None:
    """
...
def get(url: str,
        ...
        headers: dict = None,
        timeout: int = 2,
        log: Union[info, error, warning] = None) -> Union[requests.Response, None]:
    if params:
        url += f"?{urlencode(params)}"
    if log:
        _log(message=f"GET {url}", level=log)
    try:
...
def post(url: str,
    ...
    if params:
        url += f"?{urlencode(params)}"
    if log:
        _log(message=f"POST {url}, Body: {data}", level=info)
    try:
...
def put(url: str,
    ...
    if params:
        url += f"?{urlencode(params)}"
    if log:
        _log(message=f"PUT {url}, Body: {data}", level=info)
    try:
...
def delete(url: str,
    ...
    if params:
        url += f"?{urlencode(params)}"
    if log:
        _log(message=f"DELETE {url}", level=info)
    try:
